...
- **LAN** (`lan_sync_service.dart`): host opens an ephemeral `dart:io` `HttpServer` bound to `anyIPv4` (listens on every interface), shows a QR (`{v:2, hosts:[…all private LAN IPs…], port, token}` payload) + 6-digit pairing code; client scans (`mobile_scanner`, mobile-only) or enters manually, then `pickReachableHost` TCP-probes the candidate IPs (1.5s each) to find the one on the **shared subnet** — the host never guesses which interface the peer is on; the peer's connectivity is the ground truth (this sidesteps multi-NIC / hotspot / stale-IP ambiguity without needing iOS multicast entitlements). Then **pull → merge → push** so both ends end up with the union in one session. `LanServerInfo.ips` is `_privateCandidates()` (all private IPv4s, ranked wireless > wired > virtual purely to order the peer's connection attempts — correctness no longer depends on the ranking). Sync payload is a zip of `snapshot.json` + `attachments/*`. Token goes in the `x-sync-token` header. `parse` stays back-compat with the old v1 single-`host` payload. Native-only (web can't host); Windows triggers a firewall prompt on first listen.
